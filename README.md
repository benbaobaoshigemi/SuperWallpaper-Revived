...
- 覆盖 Filament 系（地球 / 月球）与 Unity 系（火星 / 土星 / 几何）、雪山场景。
...
3. 场景包（`SuperWallpaper` 基类）：息屏路径按引擎当前状态改写 `ForceLock` 事件——
   ...
   - 月球场景额外拦截唤醒路径的 `ForceLock`，避免位相重随机 / 自转重启导致的样式刷新。
...
- 本模块只做行为拦截，不包含任何来自 MIUI / 小米的代码或资源。

## 免责声明

本项目源于对设备内置专有软件行为的逆向分析，仅用于个人学习研究。请遵守所在地法律与相关软件许可，因使用本项目产生的任何后果由使用者自行承担。
